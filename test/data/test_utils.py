...
def test_train_valid_split():
    ...
    train_loader, valid_loader = train_valid_split(
        dataset, split_pct=0.8, method="partition"
    )
    ...
    assert torch.equal(train_dataset[0]["X"].squeeze(), torch.tensor([1, 2]).float())
    assert torch.equal(train_dataset[0]["y"], torch.tensor([3]).float())
    assert torch.equal(train_dataset[1]["X"].squeeze(), torch.tensor([2, 3]).float())
    assert torch.equal(train_dataset[1]["y"], torch.tensor([4]).float())
    assert torch.equal(train_dataset[2]["X"].squeeze(), torch.tensor([3, 4]).float())
    assert torch.equal(train_dataset[2]["y"], torch.tensor([5]).float())
    assert torch.equal(valid_dataset[0]["X"].squeeze(), torch.tensor([4, 5]).float())
